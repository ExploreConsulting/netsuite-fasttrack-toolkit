...
(function (lazy) {
    ...
    lazy.log = LogManager.getLogger('lazy');
    /**
    ...
    SearchIterator.prototype.moveNext = function () {
        ...
        if (this.isLastPage && this.currentIndex >= lastIndex) {
            lazy.log.debug("on last page, current index:" + this.currentIndex + " >= lastIndex:" + lastIndex);
            return false;
        }
            ...
            lazy.log.debug("loading results from " + newStartIndex + " to " + (newStartIndex + this.sliceSize));
    ...
    Lazy.Sequence.prototype.nsSearchResult2obj = function (getText, useLabels) {
        ...
        return this.map(function (r) {
            return Lazy(columns).reduce(function (acc, column) {
                ...
                if (getText === true)
                    acc[thefield + "Text"] = r.getText(column);
                return acc;
            }, { id: r.getId() }); // always include an 'id' field for row internal id
        });
    };
    function createSearch(recordType, filters, columns) {
        return Lazy().createSearch(recordType, filters, columns);
    }
    lazy.createSearch = createSearch;
    function loadSearch(recordType, id) {
        return Lazy().loadSearch(recordType, id);
    }
    lazy.loadSearch = loadSearch;
    function fromSearch(searchObj) {
        return Lazy().fromSearch(searchObj);
    }
    lazy.fromSearch = fromSearch;
    function getTotalRecordCount(recordtype, filters) {
        ...
        if (results && results.length === 1)
            ...
            lazy.log.error('getTotalRecordCount()', 'failed to determine the number of records in the result set');
            return results;
        }
    }
    lazy.getTotalRecordCount = getTotalRecordCount;
})(lazy || (lazy = {}));
